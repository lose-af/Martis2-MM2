onEvent('recipes', event => {
	...
	//event.recipes.createoreexcavation.drilling([Item.of('minecraft:coal'), Item.of('minecraft:diamond').withChance(0.05)], '{"text": "我的煤矿脉"}', 2, 1000).drill('createoreexcavation:diamond_drill').fluid('minecraft:lava').alwaysFinite().veinSize(5, 8).id("my_vein2");
	...
	//event.recipes.createoreexcavation.drilling('minecraft:raw_iron', '{"text": "我的铁矿脉"}', 10, 100).veinSize(3, 8.5).biomeWhitelist('forge:is_overworld').stress(512).id("my_vein3");
	//biomeBlacklist is also available

	//Fluid extractor recipes (Fluids)
	//event.recipes.createoreexcavation.extracting(Fluid.of('minecraft:water', 400), '{"text": "水井"}', 10, 100).alwaysInfinite().id("test");
